...
const getProductImages = async function(data){
  ...
  const getRelatedProduct = async function (data) {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/products?pageSize=4&category=${category}`);
      ...
      products.map((item)=>{
        ...
        relatedDivItem.classList.add("w-[270px]", "h-[322px]", "flex", "flex-col", "gap-[8px]", "relative", "group");
        
        // Şəkil divi yaratmaq
        const relatedDivItemImg = document.createElement('div');
        relatedDivItemImg.classList.add("h-[250px]", "w-[270px]", "p-4", "flex", "items-center", "justify-center", "rounded-md", "bg-gray-100", "relative", "overflow-hidden", "group");
        ...
        hoverEffectDiv.classList.add("absolute", "inset-x-0", "bottom-[-10px]", "bg-[#000000]", "bg-opacity-75", "opacity-0", "flex", "items-center", "justify-center", "translate-y-0", "group-hover:translate-y-[-10px]", "group-hover:opacity-0", "group-hover:opacity-100", "group-hover:transition-all", "group-hover:duration-300");
        
        // Add to basket düyməsini yaratmaq
        const addButton = document.createElement('button');
        addButton.classList.add("font-poppins", "font-medium", "text-[16px]", "leading-[24px]", "text-[#FFFFFF]", "bg-transparent", "w-[115px]", "h-[48px]", "rounded-md", "text-center");
        ...
        relatedDivItemInfo.classList.add("w-[201px]", "h-[56px]", "flex", "flex-col", "gap-[8px]");
        
        const productTitle = document.createElement('h3');
        productTitle.classList.add("font-poppins", "font-medium", "text-[16px]", "leading-[24px]", "text-[#000000]");
        productTitle.textContent =  item.title;
        
        const productPrice = document.createElement('p');
        productPrice.classList.add("font-poppins", "font-medium", "text-[16px]", "leading-[24px]", "text-[#DB4444]");
        productPrice.textContent =  `${parseFloat(
          item.price
